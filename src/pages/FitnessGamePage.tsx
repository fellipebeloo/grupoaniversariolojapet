...
import { IphoneAlarm } from '@/components/IphoneAlarm';
...
const FitnessGamePage = () => {
  ...
  const isAlarmActive = gameState === 'playing' && currentQuestionIndex === 0 && !alarmStopped;

  const containerClasses = `w-full max-w-md mx-auto bg-[#202c33] rounded-lg shadow-lg transition-all duration-500 ${
    isAlarmActive ? 'p-0 h-[600px]' : 'p-6'
  }`;
  ...
  return (
    <div className="min-h-screen bg-[#0f1418] text-white flex flex-col items-center justify-center p-4">
      <div className={containerClasses}>
        {isAlarmActive ? (
          <IphoneAlarm onStop={handleStopAlarm} />
        ) : (
          <>
